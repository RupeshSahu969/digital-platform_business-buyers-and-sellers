    ...
    <div className="bg-white rounded-lg shadow-sm p-4 h-fit">
      ...
      {Object.entries(groupedFields).map(([group, fields]) => (
        <div key={group} className="mb-4">
          <h3 className="text-xs font-medium text-gray-500 uppercase tracking-wider mb-2">{group}</h3>
          <div className="grid grid-cols-2 gap-2">
            {fields.map((field) => (
              <button
                key={field.type}
                onClick={() => addField(field.type)}
                className="flex flex-col items-center justify-center p-2 text-xs border border-gray-200 rounded-md hover:bg-indigo-50 hover:border-indigo-200 transition-colors"
              >
                <div className="text-indigo-600 mb-1">{field.icon}</div>
